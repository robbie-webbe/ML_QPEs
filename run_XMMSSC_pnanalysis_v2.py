    ...
    if cat[1].data.field('CCDPN')[i] != -32768 and cat[1].data.field('PN_ONTIME')[i] >= 30000:
        ...
        if pred_50[1] > 0.999 and pred_250[1] > 0.9999 and pred_1000[1] > 0.999:
            top_cands.append([srcid,obsid,cat[1].data.field('SRC_NUM')[index],cat[1].data.field('EP_ONTIME')[index],
                                'PNX',np.mean(lc_50.countrate),pred_50[1],pred_250[1],pred_1000[1]])
            #and save the plots to a folder
            outfile_name = obsid+'_'+srcnum+'_'+srcid+'_pnX.pdf'
            ...
            if pred_50[1] == 1.0 and pred_250[1] == 1.0 and pred_1000[1] == 1.0:
                fig.savefig('4XMMSSC/fixneg_results/top_cand_plots/conf_1/'+outfile_name)
            else:
                fig.savefig('4XMMSSC/fixneg_results/top_cand_plots/conf_cut/'+outfile_name)
            plt.close()
...
predictions_dt50.to_csv('4XMMSSC/predictions_fixneg_pndt50.csv')
predictions_dt250.to_csv('4XMMSSC/predictions_fixneg_pndt250.csv')
predictions_dt1000.to_csv('4XMMSSC/predictions_fixneg_pndt1000.csv')
top_candidates.to_csv('4XMMSSC/top_QPE_cands_fixneg_pn.csv')
...
features_dt50.to_csv('4XMMSSC/features_fixneg_pndt50.csv')
features_dt250.to_csv('4XMMSSC/features_fixneg_pndt250.csv')
features_dt1000.to_csv('4XMMSSC/features_fixneg_pndt1000.csv')
